...
CREATE OR REPLACE FORCE EDITIONABLE VIEW VW_SERVICES_PROVIDED (
    "APP_ID", 
    "Code", 
    "Service",
    "Duration",
...
SELECT
    ...
    "SPR_DURATION",
    CASE 
        WHEN "INV_ID" = 2 THEN "SER_PRICE_PER_UNIT"
        WHEN "INV_ID" = 1 THEN "SER_PRICE_PER_UNIT" * ("SPR_DURATION" / "TIM_INCREMENT")
    END AS price
FROM CRP_SERVICE_PROVIDED
JOIN CRP_SERVICE ON SPR_SER_CODE = SER_CODE
LEFT JOIN CRP_TIME_UNIT ON SER_TIM_ID = TIM_ID
LEFT JOIN CRP_INVOICE_UNIT ON SER_INV_ID = INV_ID;
/

CREATE OR REPLACE FORCE EDITIONABLE VIEW VW_PRESCRIPTIONS (
    "PRE_APP_ID", 
    "PRE_MED_ID", 
    "Medicine",
    "Prescription"
...
SELECT 
    ...
    MED_NAME,
    PRE_DESCRIPTION  
FROM CRP_PRESCRIPTION
JOIN CRP_MEDICATION ON pre_med_id = med_id;
/
EXIT
